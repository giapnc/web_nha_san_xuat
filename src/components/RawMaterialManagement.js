...
const RawMaterialManagement = () => {
  ...
  const activeIngredients = [
    ...new Set(products.map(p => p.activeIngredient).filter(Boolean)),
  ]
  ...
  const handleUpdateStatus = async (id, status) => {
    if (window.confirm(`Bạn xác nhận đổi trạng thái lô này thành ${status}?`)) {
      try {
        const response = await manufacturerService.updateRawMaterialStatus(
          id,
          status,
          "",
        )
              ...
              filteredMaterials.map(rm => (
                <tr key={rm.id}>
                  ...
                  <td>
                    <strong
                      style={{
                        color: rm.quantity > 0 ? "#059669" : "#dc2626",
                      }}>
                      {rm.quantity} {rm.unit}
                    </strong>
                  </td>
                  <td>
                    {formatDate(rm.manufactureDate)} <br />{" "}
                    {formatDate(rm.expiryDate)}
                  </td>
                  <td>
                    <span
                      className={`status-badge status-${rm.status.toLowerCase()}`}>
                      {rm.status}
                    </span>
                  </td>
                  <td>
                    {rm.status === "PENDING" && (
                      <div className="action-buttons">
                        <button
                          onClick={() => handleUpdateStatus(rm.id, "APPROVED")}
                          className="btn-approve"
                          title="Duyệt QC Đạt">
                          <Check size={16} />
                        </button>
                        <button
                          onClick={() => handleUpdateStatus(rm.id, "REJECTED")}
          ...
          <div
            ...
            <div className="modal-body form-grid">
              ...
              <div className="form-group">
                ...
                {activeIngredients.length === 0 && (
                  <div
                    ...
                    * Chưa có Hoạt chất nào được định nghĩa trong "Dòng sản
                    phẩm". Vui lòng tạo sản phẩm trước.
                  </div>
                )}
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input
                  type="date"
                  value={newMaterial.manufactureDate}
                  onChange={e =>
                    setNewMaterial({
                      ...newMaterial,
