...
const BatchProductManagement = () => {
    ...
    const fetchBatches = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/batches/with-items`,
                {
    ...
    const handleRecallConfirm = async () => {
        ...
        setRecalling(true);
        try {
            const response = await manufacturerService.updateBatchStatus(batchToRecall.id || batchToRecall.batchId, "RECALLED", recallReason);
            const txHash = response.data?.transactionHash || response.transactionHash;

            setShowRecallModal(false);

            // Hiển thị thông báo thành công với TxHash
            alert(`Đã phát lệnh thu hồi lô thuốc thành công!\nMã giao dịch Blockchain: ${txHash || 'Đang xử lý...'}`);
                                    ...
                                    <div className="batch-actions-wrapper">
                                        ...
                                        <div className="actions-dropdown">
                                            ...
                                            <button
                                                ...
                                                onClick={() => handleDownloadAllQR(batch.id || batch.batchId, batch.batchNumber)}
                                                disabled={!batch.itemsCount || batch.itemsCount === 0}
                                            >
                                                <Download size={16} />
                                            ...
                                            <button
                                                ...
                                                disabled={batch.status !== 'MANUFACTURED' || (batch.shipments && batch.shipments.length > 0)}
                                            >
                                                <Trash2 size={16} />
            ...
            {showRecallModal && batchToRecall && (
                <div className="modal-overlay">
                    <div className="delete-modal">
                        ...
                        <div className="delete-modal-body">
                            <p style={{ textAlign: 'center', color: '#6b7280', fontSize: '0.9rem', marginBottom: '20px' }}>
                                Bạn đang thực hiện lệnh thu hồi khẩn cấp cho lô thuốc này.
                            </p>
                            <div className="batch-info-to-delete" style={{ backgroundColor: '#fff1f2', borderColor: '#fecaca' }}>
                                <p><strong>Tên thuốc:</strong> {batchToRecall.drugName}</p>
                                <p><strong>Số lô:</strong> #{batchToRecall.batchNumber}</p>
                            </div>
                            ...
                            <div style={{ marginTop: '20px' }}>
                                ...
                                <textarea
                                    style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.9rem', minHeight: '100px', outline: 'none' }}
                            ...
                            <div className="warning-message" style={{ marginTop: '16px' }}>
                                <AlertCircle size={20} />
                                <span>CẢNH BÁO: Lệnh thu hồi sẽ ngăn chặn mọi giao dịch của lô này trên chuỗi cung ứng!</span>
                            </div>
                        </div>
                        <div className="delete-modal-actions">
                            <button
